...
\begin{lemma}
    Siano $A = (V_A, \to_A, a), B = (V_B, \to_B, b)$ due APG con decorazione che rappresentano lo stesso insieme (poichè per AFA la decorazione di ogni APG è unica non la indichiamo in alcun modo).
    Allora la relazione binaria $\equiv$ su $V_A \times V_B$ definita come
    \begin{gather*}
        a \equiv b \iff ``\text{le decorazioni di } A,B \text{ associano ad } a,b \text{ lo stesso insieme}''
    \end{gather*}
    è una bisimulazione su $A,B$.
\end{lemma}
\begin{proof2}
    Dimostriamo innanzitutto che $\equiv$ è una bisimulazione sui grafi $G_A = (V_A, \to_A), G_B = (V_B, \to_B)$. Chiaramente se $a \equiv b, a \to a'$ si ha che
    \begin{itemize}
    ...
    La relazione $\equiv$ è una bisimulazione sugli APG $A,B$ quando si considera per ipotesi che $A,B$ rappresentano lo stesso insieme.
\end{proof2}
\begin{proof2}(del Teorema \ref*{theo:bisi_iff_eqsets})
    L'implicazione ($\impliedby$) è una conseguenza diretta del Lemma appena dimostrato.\\
    $\implies$ ?
\end{proof2}
