...
\end{example}
\begin{definition}
    Siano $X_1,X_2$ due partizioni dello stesso insieme. Diremo che $X_2$ è \emph{più fine} rispetto a $X_1$ se $\forall x_1 \in X_1, \,\,\forall x_2 \in X_2$ si ha $x_2 \subseteq x_1$.
\end{definition}
\begin{example}
...
\begin{definition}
    Sia $A$ un insieme. Sia $R$ una relazione. Diremo che una partizione $X$ è la RCSP di $A$ rispetto ad $R$ se, di tutte le partizioni di $A$ stabili rispetto
    ad $R$, è quella contenente il minor numero di blocchi (per questo motivo si dice che è la \emph{più rozza}).
\end{definition}
\begin{example}
...
\subsubsection{Legame con la bisimulazione}
Per presentare il legame tra RSCP e bisimulazione diamo innanzitutto la seguente definizione (come in \cite{gentilini}):
\begin{definition}
