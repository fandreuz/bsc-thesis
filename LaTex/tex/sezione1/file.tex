...
\end{example}
Gli insiemi non-ben-fondati non rientrano all'interno della teoria degli insiemi ZFC, in quanto non rispettano il seguente Assioma:
\begin{axiom}[di fondazione]
    Ogni insieme non vuoto contiene un elemento disgiunto dall'insieme stesso.
    \label{axi:foundation}
\end{axiom}
Un'implicazione equivalente dell'Assioma \ref*{axi:foundation} è la seguente: ``$\forall A$ la relazione $\in$ è ben-fondata su $A$'' \cite[Chapter III.4]{kunen}, che implica l'impossibilità per un insieme di contenere sé stesso. E' chiaro che questo contrasta con la definizione stessa di insieme non-ben-fondato.\\
