...
L'algoritmo che presentiamo in questa sezione (proposto da Paige e Tarjan in \cite{paigetarjan}) risolve il problema generale presentato all'inizio di questo lavoro, e generalizza il procedimento di Hopcroft sfruttandone l'intuizione. Migliora l'algoritmo $O(mn)$ proposto da Kanellakis e Smolka nel 1983 (non trattato in questo lavoro), dove $m$ è il numero di coppie di nodi messe in relazione (ovvero la cardinalità di ``$\to$'') ed $n$ è il numero di nodi.
