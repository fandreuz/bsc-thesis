...
\begin{algorithm}[H]
    ...
    \Begin{
        \tcp*[h]{Inizializziamo $\delta^{-1}$}\\
        \ForAll{$(s,i) \in S \times I$}{
            $\delta^{-1}(s,i) \coloneqq \{t : \delta(t,i) = s\}$\;\label{alg:hop_deltainverso}
        }
        $B(1) \coloneqq F, B(2) \coloneqq S - F$\;
        \tcc*[h]{Per ogni ingresso $i$ costruiamo l'insieme degli stati in $B(j)$ aventi controimmagine non vuota rispetto  a $\delta_i$}\\
        \ForAll{$j \in \{1,2\}$}{
            \ForAll{$i \in I$}{
        ...
        \While{$\exists i \in I : L(i) \neq \emptyset$}{
            ...
            \ForAll{$m \leq k : \exists t \in B(m) : \delta(t,i) \in i(j)$}{
                $B^{'}(m) \coloneqq \{u \in B(m) : \delta(u,i) \in i(j)\}$\;
...
\end{algorithm}
Nella Riga \ref{alg:hop_deltainverso} definiamo $\delta^{-1}$, una funzione (che nella pratica può essere un array bidimensionale, supponendo numerati univocamente gli stati e gli ingressi) che consente l'accesso in tempo costante all'insieme degli stati che portano in un determinato stato dopo un determinato ingresso. Come si può osservare questo sarà fondamentale nel seguito del procedimento.\\
...
Per evitare accessi inutili (che incrementerebbero i termini costanti dell'algoritmo, con conseguente peggioramento del tempo di esecuzione su input piccoli) nella Riga \ref{alg:hop_stati_delta_nonvuoto} definiamo $i(j)$ per $i \in I, j \in \{1,2\}$ (nella pratica può essere un array bidimensionale) come l'insieme degli stati $s$ nel blocco $B(j)$ per cui esiste qualche stato $t$ tale che $\delta(t,i) = s$.\\
All'interno del ciclo della Riga \ref{alg:hop_splitters} definiamo gli insiemi $L(i)\,\, \forall i \in I$, che contengono gli indici dei blocchi che verranno usati come \emph{splitter} nel seguito del procedimento. Vale la seguente definizione:
\begin{definition}
