...
Descriveremo innanzitutto un algoritmo \emph{naive} avente complessità $O(mn)$, dove $m$ è il numero di coppie di nodi messe in relazione (ovvero la cardinalità di ``$\to$'') ed $n$ è il numero di nodi. Sulla base di questo applicheremo i miglioramenti sviluppati dall'idea di Hopcroft, che consentiranno di ridurre la complessità ad $O(m\log n)$. Come osserveremo nel seguito, l'idea generale consiste nello scegliere in modo oculato gli \emph{splitter} con cui rifinire la partizione.
