...
def dfs_rank_visit(graph_scc, current_scc):
    ...
    # current_scc is a leaf of graph_scc
        ...
        for node in graph_scc.adj[current_scc]:
            ...
            current_max = max(current_max, node_rank + 1 if graph_scc.nodes[node]['wf'] else node_rank)
...
def prepare_scc(graph):
    ...
    components = map(lambda scc: frozenset(scc), nx.strongly_connected_components(graph))
...
def compute_rank(graph):
    ...
    for scc_node in filter(lambda node: 'rank' not in graph_scc.nodes[node], graph_scc.nodes):
