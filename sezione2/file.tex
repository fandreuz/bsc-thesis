...
\end{proposition}
\begin{proof2}
    Per il Lemma \ref{lem:pt_qx}, la partizione $Q$ è sempre stabile rispetto ad ogni blocco di $X$. Ma, poichè la condizione di terminazione è $Q=X$, $Q$ è stabile. Se consideriamo il risultato restituito dall'algoritmo, cioè $Q \cup \widetilde{S}''$, è evidente che tale partizione è ancora stabile, visto che $\forall B \in Q, \forall C \in \widetilde{S}'', C \cap R^{-1}(B) = \emptyset, $
\end{proof2}
