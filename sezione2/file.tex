...
\end{proof2}
\begin{observation}
    La relazione ``$\sim$'' con la formulazione dell'Osservazione \ref{obs:equivalent_states} è una bisimulazione sull'insieme degli stati di un'automa, se si considera la relazione binaria $\displaystyle E \,\,\coloneqq \bigcup_{i \in I, x \in S} \{(x,\delta(x,i))\}$.
\end{observation}
\begin{proof2}
    Supponiamo che $x \sim y$. Sia $x E x'$, cioè $\exists i \in I : x' = \delta(x,i)$. Sia $y' = \delta(y,i) \implies y E y'$. Allora, per l'Osservazione \ref{obs:equivalent_states}, $x' \sim y'$.\\
...
\begin{algorithm}[H]
    ...
    \Begin{
        $\widetilde{S}', \widetilde{S}'' \coloneqq $ SeparaNodiNonIntraprendenti$(S,R,\widetilde{S})$\; \label{alg:pt_nodi_ni}
        $Q \coloneqq \widetilde{S}', X \coloneqq \{S - \bigcup_{A \in \widetilde{S}''} A\}$\;
        \While{$Q \neq X$}{
            Scelgo in modo casuale un blocco $B \in X \mid B \not\in Q$\;
            Scelgo un blocco $C \in X \mid C \subset B, |C| \leq |B|/2$\;
            $X = (X - \{B\}) \cup \{C, B-C\}$\;
            $Q = (Q \,- \{C\}) \,\cup \splitfunc(B-C, \splitfunc(C,Q))$\;
        }
        \Return{$Q \cup \widetilde{S}''$}\;
    }
    \SetKwProg{Def}{def}{:}{}
    \Def{\textup{SeparaNodiNonIntraprendenti}$(S,R,\widetilde{S})$}{
        $I = \emptyset, NI = \emptyset$\;
        \ForAll{$B \in \widetilde{S}$}{
            $B' = B \cap R^{-1}(S)$\;
            $B'' = B - B'$\;
            $I = I \cup B', NI = NI \cup B''$\;
        }
        \Return{$I,NI$}\;
    }
\end{algorithm}
L'algoritmo prende in input un insieme $S$, una relazione binaria $R$ su $S$ ed una partizione iniziale $\widetilde{S}$. La prima operazione (Riga \ref{alg:pt_nodi_ni}) consiste nell'isolare, per mezzo della funzione ``SeparaNodiNonIntraprendenti'', i nodi non \emph{intraprendenti}, cioè che non intraprendono (come primo elemento della relazione) alcun legame tramite $R$ con un altro elemento di $S$. La partizione iniziale $\widetilde{S}$ viene quindi divisa in due sottopartizioni, quella dei blocchi del tipo $B'$ e quella dei blocchi del tipo $B''$. Motiveremo questo processamento preliminare nel seguito.

L'algoritmo procede con la definizione di due partizioni $Q,X$: la prima è la sottopartizione formata dai blocchi \emph{intraprendenti} ricavati da $\widetilde{S}$, mentre la seconda è la partizione banale formata da un unico blocco contenente tutto $S$, escluendo però i nodi non \emph{intraprendenti}, che da questo in punto in poi non saranno più presi in considerazione. Le relazioni invarianti che intercorrono tra queste due partizioni sono fondamentali per la complessità migliorata del procedimento.
...
Quando $Q$ e $X$ coincidono, viene restituita la partizione di $S$ formata da $Q$ e dai blocchi del tipo $B''$, cioè la RSCP$(\widetilde{S},R)$.

\subsubsection{Analisi}
Dimostriamo innanzitutto la validità della seguente invariante:
\begin{lemma}
    \label{lem:pt_qx}
    All'inizio di ogni iterazione del ciclo la partizione $Q$ è stabile rispetto ad ogni blocco di $X$. Inoltre $Q$ rifinisce $X$.
\end{lemma}
\begin{proof2}
    Procediamo per induzione. Naturalmente l'invariante è valida prima della prima iterazione, in quanto $Q$ è composto da tutti e soli gli elementi \emph{intraprendenti}, e quindi per ogni blocco $C \in Q = \widetilde{S}'$ si ha $C \subset R^{-1}(S)$.

    Ora supponiamo che l'invariante sia valida prima di un'iterazione qualsiasi, e dimostriamo che resta valida al termine della stessa. Questo fatto è una conseguenza diretta del modo in cui l'algoritmo modifica $Q,X$: in $X$, infatti, viene sostituito il blocco $B$ con i blocchi $B-C, C$, mentre $Q$ viene modificato da sue applicazioni successive della funzione \splitfunc. E' evidente che $Q$ deve essere stabile rispetto a $B-C, C$, ed anche a tutti gli altri blocchi di $X$ per l'ipotesi induttiva. Inoltre, sempre per l'ipotesi induttiva, devono esistere (prima della modifica) dei blocchi $C_1, \dots, C_n \mid \bigcup_{i=1}^n C_i = B$. Dopo la modifica di $Q$ l'unione dei nuovi blocchi generati dall'applicazione dei due \splitfunc ai blocchi $\{C_1, \dots, C_n\} - C$ è $B-C$, mentre l'unione dei due nuovi blocchi (di cui uno è eventualmente vuoto) generati a partire da $C$ è chiaramente $C$.
\end{proof2}
