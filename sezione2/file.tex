...
\begin{theorem}
    ...
    \end{gather*}
    in cui i blocchi il cui indice $i_j$ è contenuto in $L(i)$ contribuiscono con $|i(i_j)|\log_2 |i(i_j)|$, mentre i restanti blocchi contribuiscono con $|i(i_k)|\log_2 \frac{|i(i_k)|}{2}$.
\end{theorem}
\begin{proof2}
    ...
    \begin{itemize}
        ...
        \begin{align*}
         ...
         \end{align*}
        dove consideriamo il tempo impiegato per l'iterazione, ed il fatto che l'indice $x$, al termine dell'iterazione, fa parte dell'insieme dei blocchi il cui indice non appartiene ad $L(i)$. Allora:
        \begin{align*}
...
Descriveremo innanzitutto un algoritmo \emph{naive} avente complessità $O(|R||A|)$. Sulla base di questo applicheremo i miglioramenti sviluppati dall'idea di Hopcroft, che consentiranno di ridurre la complessità ad $O(|R|\log |A|)$. Come osserveremo nel seguito, l'idea generale consiste nello scegliere in modo oculato gli \emph{splitter} con cui rifinire la partizione.
